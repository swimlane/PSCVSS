...
elseif ($env:APPVEYOR_PULL_REQUEST_NUMBER -gt 0) {
    ...
    Try {
        ...
        (Get-Content -Path $manifestPath) -replace 'PSGet_PSCVSS', 'CaptureIT' | Set-Content -Path $manifestPath
    ...
    Import-Module -Name "$PSScriptRoot\PSCVSS" -Force
    ...
    Try {
        ...
        $PM = @{
            ...
            ErrorAction = 'Stop'
            Tags         = 'CVSS', 'PowerShell', 'PowerShell Core', 'Vector', 'Vulnerability'
            LicenseUri   = 'https://github.com/Swimlane/PSCVSS/blob/master/LICENSE'
            ProjectUri   = 'https://github.com/Swimlane/PSCVSS'
            ReleaseNotes = 'Initial release to the PowerShell Gallery'
        }

        Publish-Module @PM
        Write-Host "PSCVSS PowerShell Module version $newVersion published to the PowerShell Gallery." -ForegroundColor Cyan
    }
    Catch {
        # Sad panda; it broke
